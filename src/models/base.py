...
def compile_create_table(element, compiler, **kw):
    ...
    if table.info.get("is_hypertable", False):
        models_logger.info(f"Creating hypertable for table {table.name} with interval {table.info['hypertable_interval']}")
...
def init_db():
    try:
        ...
        with engine.connect() as conn:
            ...
            models_logger.info("TimescaleDB extension created (if it didn't already exist)")
